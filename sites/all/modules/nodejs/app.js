...
var publishMessage = function (request, response) {
  ...
  request.on('end', function () {
    ...
    //setTimeout(function(){console.log("Hello")}, 3000);
      ...
      //console.log('setcount' +sentCount);    
    }
...
var publishMessageToChannel = function (message) {
  ...
  if(message['resendCallback'] != undefined && message['resendCallback'] != null) {
    ...
    var resendCallback = function(resendCallbackURL) {
      //console.log("callback delay==>" + callbackDelay);
      setTimeout(function() {
      //  console.log("callback delay==>" + callbackDelay);
  ...
  //console.log("Message==> " + message.data.users.length);
 nid = message.data.nid;
  userArr = message.data.users;
  if (message.data.users.length > 1) {
      console.log("users count more then one");
      console.log("NID==> " + nid);
      console.log("user array length==> " + userArr.length);
      howManyTimes = userArr.length;
      var sessCount = 0;
      var options = {
          hostname: 'chpride.com',
          port: 80,
          path: '/ridestatus?nid=' + nid,
          method: 'GET'
      };
      // End ride request if accepted
      sendNodejsMessage(sessCount);

      function sendNodejsMessage(sessCount) {
          // HTTP request block start
          var req = https.request(options, function(res) {
              res.setEncoding('utf8');
              res.on('data', function(chunk) {
                  console.log('Chunk==>>' + chunk);
                  if (chunk == '1') {
                      //Send Ride request to next driver
                      console.log("I  " + sessCount);
                      sessionId = getNodejsSessionIdsFromUid(parseInt(userArr[sessCount]));
                      ...
                      //setTimeout( sendRidequest, 10000, nid);
                      sessCount++;
                      if (sessCount < howManyTimes) {
                          setTimeout(sendNodejsMessage, 10000, sessCount);
                      }
      ...
      console.log("user array length==> " + userArr.length);
      nid = message.data.nid;
      sessionId = getNodejsSessionIdsFromUid(parseInt(userArr));
      console.log("sessionId==> " + sessionId);
      if (sessionId != '') {
        for (i = 0; i < sessionId.length; i++) { 
          if (publishMessageToClient(sessionId[i], message)) {
              clientCount++;
          }
        }
...
var getNodejsSessionIdsFromUid = function (uid) {
  ...
  for (var sessionId in io.sockets.sockets) {
    console.log("io.sockets.sockets[sessionId].uid: " + io.sockets.sockets[sessionId].uid + "Session: " + sessionId)
    //sessionIds.push(sessionId + "," + io.sockets.sockets[sessionId].uid);
...
var addUserToChannel = function (request, response) {
  console.log("addUserToChannel==> " + request.params.uid)
...
//console.log("settings.baseAuthPath + settings.publishUrl " + settings.baseAuthPath + settings.publishUrl)
